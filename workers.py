    ...
    try:
        ...
        while not work_queue.empty():
            if not run.value:
                ...
                # print(f"worker: queue not epmpty")
                
                # if len(erbehandlet) > 30:
                #     break
                try:
                    # print(f" worker:  getting from queue")
                    url = work_queue.get(timeout=1)
                    # print(f" worker:  {url}") 
                except Empty:
                    # print(f" worker:  in except Empty")
                    empty+=1
                ...
                for url in page.links:
                    # print(f"{url} in tobeprocessed: {url in tobeprocessed}")
                    # print(f"{config['domain']} in url: {config['domain'] in url}")
                    # print(f"{url} in negate: {any(word in url for word in negate)}")
...
def table_worker(queue: Queue, table: QTableView, num_workers: int, antall: QLabel, isprocessed: dict, tobeprocessed: dict):
    ...
    while True:
        try:
            ...
            antall.setText(f"Is processed: {len(isprocessed)}\nTo be processed: {len(tobeprocessed)}\nIn table: {currentRowCount}")
...
def run_page_workers(first: str, num_workers: int, table: QTabWidget, antall: int, run: Value):
    ...
    try:
        ...
        for i in workers:
            # print(f"joining {i}")
            i.join()
        antall.setText(f"Number of urls: {table.rowCount()}")
    except Exception as e:
        print(e)
